...
export const checkIfTableExists = async (tableName) => {
  ...
  try {
    ...
    // const tableList = await sequelize.getQueryInterface().showAllSchemas();
    //   tableList.map(table=>{
    //     console.log(table);
    //   });

    // const tableExists = tableList.some(schema => schema.tableName === tableName);
// console.log("table existance status: " + tableExists);
    // return tableExists;
