...
class InvitationEmailService{
    ...
    generateInvitationLink(workspaceId, recipientEmail, token) {
        return `${config.hostname}/join-workspace/${workspaceId}?invite=${recipientEmail}&token=${token}`;
      };
    ...
    async sendWorkspaceInvitation(recipientEmail, workspaceName, invitationLink) {
        const mailOptions = {
          ...
          html: `
            ...
            <p>Click <a href="${invitationLink}">here</a> to join. This invitation link will be expire after 24 hours.</p>
            <p>Thank you!</p>
          `, // HTML body
        };
